...
export default async ({ source, target, file, environment = "production", prebuild = false }) => {
  ...
  let pages = await Promise.all(
    componentSourceFiles.map(async (sourceFile) => {
      ...
      return {
        ...
        relativePath: sourceFile.relativePath, // Include relativePath for matching below
      }
    }),
  )

  for (const sourceFile of sourceFilesToBuild) {
    const page = pages.find((page) => page.relativePath === sourceFile.relativePath); // Find page object
    const includeForPrebuild = page && page.meta.prebuild; // Check if page should be included in prebuild
    const shouldBuild = (prebuild && includeForPrebuild) || (!prebuild && !includeForPrebuild); // Check if page should be included in current build

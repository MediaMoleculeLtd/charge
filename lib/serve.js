...
export default async ({ source, openBrowser = true, port = 2468 }) => {
  ...
  // Admin site middleware. SPA at /admin/index.html will handle all admin routes
  const handleAdmin = function(request, response, next) {
    const { path } = parseURL(request.url);
  ...
  browserSync.watch([`${target}/**/*.css`, `${target}/**/*.js`], { ignoreInitial: true }, async (event, file) => {
    // Currently just reloading CSS and JS files as our site content is not written in the way Charge is expecting
  ...
  return new Promise((resolve) => {
    browserSync.init(
      {
        logLevel: "silent",
        middleware: [redirectWithoutTrailingSlash, apiProxy, handleAdmin, tryDirectoryIndex, tryIndexPage],
        open: openBrowser,
        port: port,
        server: {
          baseDir: target,
